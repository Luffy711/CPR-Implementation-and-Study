...
void issue_queue::flush() {
	...
	for (unsigned int i = 0; i < size; i++) {
		if (q[i].valid) {
			if (proc->PAY.buf[q[i].index].A_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].A_phys_reg);
			if (proc->PAY.buf[q[i].index].B_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].B_phys_reg);
			if (proc->PAY.buf[q[i].index].C_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].C_phys_reg);
			if (proc->PAY.buf[q[i].index].D_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].D_phys_reg);
		}
		q[i].valid = false;
	}
...
void issue_queue::clear_branch_bit(uint64_t squash_mask) {
	for (unsigned int i = 0; i < size; i++) {
		CLEAR_BIT(squash_mask, q[i].checkpoint_id);
	}
}

void issue_queue::squash(uint64_t squash_mask) {
	for (unsigned int i = 0; i < size; i++) {
		if (q[i].valid && BIT_IS_ONE(squash_mask, q[i].checkpoint_id)) {
			if (proc->PAY.buf[q[i].index].A_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].A_phys_reg);
			if (proc->PAY.buf[q[i].index].B_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].B_phys_reg);
			if (proc->PAY.buf[q[i].index].C_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].C_phys_reg);
			if (proc->PAY.buf[q[i].index].D_valid) proc->REN->dec_usage_counter(proc->PAY.buf[q[i].index].D_phys_reg);
