...
void pipeline_t::retire(size_t& instret, size_t instret_limit) {
	...
	bool amo_success;
	bool load, store, csr=0; //, amo, csr;
	trap_t *trap = NULL; // Supress uninitialized warning.
	reg_t offending_PC;
	
	bool proceed;
	
	// cout<<"Retire.state = "<<RETSTATE.state<<endl;
	
	switch (RETSTATE.state) {
		case RETIRE_IDLE:
			proceed = REN->precommit(RETSTATE.chkpt_id, RETSTATE.num_loads_left, RETSTATE.num_stores_left,
			RETSTATE.num_branches_left, RETSTATE.amo, RETSTATE.csr, RETSTATE.exception);
			...
			else 
			{
			    ...
			    if (RETSTATE.exception) {   // exception
		...
		case RETIRE_BULK_COMMIT:
			...
			{
				if (RETSTATE.num_loads_left == 0) break;
				LSU.train(true);
			...
			for (int i = 0; i < retire_width; i++)
			{
				if (RETSTATE.num_stores_left == 0) break;
				LSU.train(false);
			...
			for (int i = 0; i < retire_width; i++)
			{
				if (RETSTATE.num_branches_left == 0) break;
				FetchUnit->commit();
			...
			for (int i = 0; i < retire_width; i++)
			{
				if (RETSTATE.log_reg == NXPR+NFPR) break;
