...
void pipeline_t::writeback(unsigned int lane_number) {
   unsigned int index;
   uint64_t squash_mask, total_loads = 0, total_stores = 0, total_branches = 0;
   bool next;
   ...
   if (Execution_Lanes[lane_number].wb.valid) {
	...
	// resolve(PAY.buf[index].branch_ID, 1);
	    // FIX_ME #15b END
			...
			next = true;
			squash_mask = REN->rollback(PAY.buf[index].checkpoint_ID, next, total_loads, total_stores, total_branches);
			// cout<<"instr_renamed_since_last_checkpoint: "<<instr_renamed_since_last_checkpoint<<endl;
            ...
            // FIX_ME #15d BEGIN
			// resolve(PAY.buf[index].branch_ID, 0);
