...
void pipeline_t::rename2() {
   unsigned int i;
   unsigned int index;
   unsigned int bundle_dst, bundle_branch, bundle_chkpts;
   uint64_t temp_instr_renamed_since_last_checkpoint = instr_renamed_since_last_checkpoint;
   db_t * actual;
   bool ChkptBeforeREN, ChkptAfterREN, Excp_flag, BrMp_flag;
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      // FIX_ME #1 BEGIN
		...
		// if (PAY.buf[index].good_instruction) {
			...
			// if ((actual->a_flags == F_CSR) || (actual->a_flags == F_AMO))
			// {
				// // cout<<"------------------------It is AMO/CSR Instr---------------------\n";
				// bundle_chkpts = bundle_chkpts + 2;
				// temp_instr_renamed_since_last_checkpoint = 0;
			...
			// {
				...
				// temp_instr_renamed_since_last_checkpoint = 0;
			// }
			// else if ((actual->a_next_pc != PAY.buf[index].next_pc) || (temp_instr_renamed_since_last_checkpoint == max_instr_bw_checkpoints))
			// {
				// bundle_chkpts++;
				// temp_instr_renamed_since_last_checkpoint = 0;
			// }
		...
		if (PAY.buf[index].good_instruction) {
			actual = get_pipe()->peek(PAY.buf[index].db_index);
			if (actual->a_exception) Excp_flag = true;
			else if (actual->a_next_pc != PAY.buf[index].next_pc) BrMp_flag = true;
		}
		...
		else if (BrMp_flag || (temp_instr_renamed_since_last_checkpoint == max_instr_bw_checkpoints))
		{
			bundle_chkpts++;
			temp_instr_renamed_since_last_checkpoint = 0;
		}
   ...
   // if (REN->stall_branch(bundle_branch) || REN->stall_reg(bundle_dst)) 
	   // return;
   if (REN->stall_checkpoint(bundle_chkpts) || REN->stall_reg(bundle_dst)) 
	   return;
   // FIX_ME #2 END
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      index = RENAME2[i].index;
	  ChkptBeforeREN = false;
	  ChkptAfterREN = false;
	  Excp_flag = false;
	  BrMp_flag = false;
	  
	  assert(instr_renamed_since_last_checkpoint <= max_instr_bw_checkpoints);
	  instr_renamed_since_last_checkpoint++;
	  
	  // cout<<"Before reset: "<<instr_renamed_since_last_checkpoint<<endl;
	  ...
	  else if (BrMp_flag || (instr_renamed_since_last_checkpoint == max_instr_bw_checkpoints))
	  {
		  ChkptAfterREN = true;
	  }
	  ...
	  if ((ChkptBeforeREN == true) && (instr_renamed_since_last_checkpoint != 0))
	  {
		  REN->checkpoint();
		  instr_renamed_since_last_checkpoint = 0;
		  // PAY.buf[index].checkpoint_ID = REN->get_checkpoint_ID(IS_LOAD(PAY.buf[index].flags), IS_STORE(PAY.buf[index].flags), IS_BRANCH(PAY.buf[index].flags), IS_AMO(PAY.buf[index].flags), IS_CSR(PAY.buf[index].flags));
	  }
