...
void lsu::store_addr(cycle_t cycle,
   ...
   try {
      ...
      unsigned int al_index = proc->PAY.buf[SQ[sq_index].pay_index].checkpoint_ID;
...
void lsu::execute_load(cycle_t cycle,
  ...
  // If either a hit or a miss and the line has already been loaded
    ...
    try{
	  ...
	  {
      unsigned int al_index = proc->PAY.buf[LQ[lq_index].pay_index].checkpoint_ID;
...
void lsu::restore(unsigned int recover_lq_tail, bool recover_lq_tail_phase,
	...
	unsigned int tail_recover_start;
	bool tail_recover_start_phase;
	
	
	if (lq_tail == 0) { 
		tail_recover_start = lq_size - 1;
		tail_recover_start_phase = !lq_tail_phase;
	} else {
		tail_recover_start = lq_tail - 1;
		tail_recover_start_phase = lq_tail_phase;
	}
	
	if (recover_lq_tail_phase == tail_recover_start_phase)
	{
		for (signed int i = recover_lq_tail; i <= tail_recover_start; i++) {
			assert(LQ[i].valid);
			if (LQ[i].addr_avail && !LQ[i].value_avail) 
			{
				if (proc->PAY.buf[LQ[i].pay_index].C_valid) proc->REN->dec_usage_counter(proc->PAY.buf[LQ[i].pay_index].C_phys_reg);
			}
		}
	}
	else
	{
		for (signed int i = 0; i <= tail_recover_start; i++) {
			// assert(LQ[i].valid);
		...
		for (unsigned int i = recover_lq_tail; i <= lq_size-1; i++) {
			// assert(LQ[i].valid);
			if (LQ[i].valid && LQ[i].addr_avail && !LQ[i].value_avail) 
			{
				if (proc->PAY.buf[LQ[i].pay_index].C_valid) proc->REN->dec_usage_counter(proc->PAY.buf[LQ[i].pay_index].C_phys_reg);
